%This script is written to plot the cost funciton in the bulk modulus for a
 ...
 %Date: 2022-10-13
  ...
  %Change Log:
    %V1- 2022-11-02: Added in preconditioning on the kinematic fields that
...
Gul=RefPar.MatProps.Gi*1.2;
Gll=RefPar.MatProps.Gi*.8;
...
%% Condition data prior to cost function
fprintf('Conditioning Kinematic Fields \n')
...
EtauDesig=strcat(ParentDesig,'_identParam');
...
EtauDesig=strcat(ParentDesig,'_exactParam');
[Etau.Phi,Etau.Phi_K,Etau.Phi_G] = func_PlotPhiConstTau(K_vec,G_vec, ...
     tau_exact,...
    SG,knownParam,strain,time,CondOpts,EtauDesig,MainSaveDir, ...
    Kexact,Gexact);
...
DataSaveName=strcat(MainSaveDir,'/',ParentDesig,'_BulkCostFunctions.mat');
save(DataSaveName)
