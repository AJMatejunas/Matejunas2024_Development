...
%V19_NoisyBulkOnly- 2023/03/28: Made spatial cropping and extrapolation
                                       %intervals the same
%V19_NoNoiseBulkOnly- 2023/04/03: Removed noise from analysis
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[phiE,phiI]=deal(zeros(TempKernNum,SpaKernNum,3)); 
extrapOpts.accel.tempPadOn=false;

%% Initialize contitutive Parameter
ConstitutiveParamE=zeros(TempKernNum,SpaKernNum,2);
ConstitutiveParamI=zeros(TempKernNum,SpaKernNum,2);
Rho=material.rho;
timefull=time;
...
%             tauE=TempTauE(TempPhiE==minPhiE);%             phiE(k,m,:)=TempPhiE';
%             ConstitutiveParamE(k,m,:)=[KE(1);tauE(1)];
            ...
            phiI(k,m,:)=TempPhiI';
            ConstitutiveParamI(k,m,:)=[KI(1);tauI(1)];
...
% 
% subplot(2,2,2)
% imagesc(SpaKernVec,TempKernVec,Errors.tau.E)
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('\tau_1 Error Exact G (%)')
% colormap(PBWYRK_map)
% colorbar
% set(gca,'YDir','normal')
% caxis([-25,25])
...
subplot(1,2,1)
imagesc(SpaKernVec,TempKernVec,Errors.K.I)
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('K_1 Error Identified G (%)')
colormap(PBWYRK_map)
colorbar
set(gca,'YDir','normal')
caxis([-25,25])
...
subplot(1,2,2)
imagesc(SpaKernVec,TempKernVec,Errors.tau.I)
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('\tau_1  Error identified G (%)')
colormap(PBWYRK_map)
colorbar
set(gca,'YDir','normal')
caxis([-25,25])
FigSaveName=strcat(savePath,'/',TestDeg,...
    '_NoNoise_Konly_Errors_imagesc');
...
caxis([-5,5])
FigSaveName=strcat(savePath,'/',TestDeg,...
    '_NoNoise_Konly_Errors_imagesc_zoomed');
...
% subplot(2,2,1)
% contourf(SpaKernVec,TempKernVec,Errors.K.E,50,'LineStyle','none')
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('K_1 Error Exact G (%)')
% colormap(PBWYRK_map)
...
% 
% subplot(2,2,2)
% contourf(SpaKernVec,TempKernVec,Errors.tau.E,50,'LineStyle','none')
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('\tau_1 Error Exact G (%)')
% colormap(PBWYRK_map)
...
subplot(1,2,1)
contourf(SpaKernVec,TempKernVec,Errors.K.I,50,'LineStyle','none')
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('K_1 Error Identified G (%)')
colormap(PBWYRK_map)
...
subplot(1,2,2)
contourf(SpaKernVec,TempKernVec,Errors.tau.I,50,'LineStyle','none')
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('\tau_1 Error Identified G (%)')
colormap(PBWYRK_map)
...
FigSaveName=strcat(savePath,'/',TestDeg,'_NoNoise_Konly_Errors_PBWYRK');
saveas(gcf,FigSaveName,'fig')
saveas(gcf,FigSaveName,'svg')
...
%         tempMatE{k,m}=tempPropsE;
        ...
        tempPropsI.tau=Ident.tau.I(k,m);
...
FigSaveName=strcat(savePath,'/',TestDeg,...
    '_NoNoise_Konly_RMSE_Phi_zoomed');
