...
%Change Log:
    ...
    %V7- 2023/01/08- Added ability to threshold the data that is used to
        %calculate phi based on the predicted noise floor in the kinematic
        %fields. The factors used to threshold the data is found in the
        %condOpts inputs
...
strainXX=squeeze(mean(strain.x));
strainS=squeeze(mean(strain.s));

%% Threshold data based on the noise floor
[xxSG,ShearSG,Avxx,Avxy,CondOpts] =...
    func_thresholdKinFields(xxSG,ShearSG,Avxx,Avxy,...
