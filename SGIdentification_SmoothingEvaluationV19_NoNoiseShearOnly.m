...
%V17_NoNoiseShearOnly- 2023/03/24: Removed Noise from Calculations
%V18_NoNoiseShearOnly- 2023/03/27: Made acceleration extrapolation kernel
                                       %the same size as the displacement 
                                       %extrapolation kernel 
%V19_NoNoiseShearOnly- 2023/03/27: Made spatial cropping and extrapolation
                                       %intervals the same
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
phi=zeros(TempKernNum,SpaKernNum,3); 
...
ConstitutiveParam=zeros(TempKernNum,SpaKernNum,2);
Rho=material.rho;
timefull=time;
extrapOpts.accel.temporalPadOn=false;
...
for k=1:TempKernNum
    ...
    parfor m=1:SpaKernNum
        ...
        [specimenN,~] = func_updateSpecGeom(specimen,gridN,disp);
        ...
        phi(k,m,:)=TempPhi';
        ConstitutiveParam(k,m,:)=[G(1);tau(1)];
    ...
    progmsg=strcat(itNum,'/',num2str(itTotal),...
        'Temporal Smoothing Iterations complete');
...
subplot(1,2,2)
imagesc(SpaKernVec,TempKernVec,Errors.tau)
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('\tau_1  Error(%)')
colormap(PBWYRK_map)
...
caxis([-5,5])
FigSaveName=strcat(savePath,'/',TestDeg,...
    '_NoNoise_Gonly_Errors_imagesc_zoomed');
...
subplot(1,2,1)
contourf(SpaKernVec,TempKernVec,Errors.G,50,'LineStyle','none')
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('G_1 Error (%)')
colormap(PBWYRK_map)
...
subplot(1,2,2)
contourf(SpaKernVec,TempKernVec,Errors.tau,50,'LineStyle','none')
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('\tau_1 Error(%)')
colormap(PBWYRK_map)
...
FigSaveName=strcat(savePath,'/',TestDeg,'_NoNoise_Gonly_Error_PBWYRK');
saveas(gcf,FigSaveName,'fig')
saveas(gcf,FigSaveName,'svg')
