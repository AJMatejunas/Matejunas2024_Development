...
ylabel('G_{tan} (Pa)')
yyaxis right
plot(t*10^6,abs(MidStrainXY))
ylim([0,1.1*max(abs(MidStrainXY))])
ylabel('Shear Strain at center')
xlabel('time (\mu{}s)')
legend('Tangent modulus','G_0','G_{\infty}','abs(Shear Strain)')

saveas(gcf,strcat(testdeg,'tangent_mod_strain.fig'));
saveas(gcf,strcat(testdeg,'tangent_mod_strain.png'));
