function phi_K=func_ViscoBulkCostSG(xxSG,knownParam,constParam,...
...
%% Function input arguments
    ...
    %knownParam- A row vector of known parameters in the following order
     ...
     %constParam- vector 
        %[K1,K2,..., Kn]; if Kinf&Ginf are known,
        %[Kinf,Ginf,K1,K2,..., Kn]; if Kinf&Ginf are unknown
...
if length(knownParam)==6
    MatProps.Gi=knownParam(5);
    MatProps.tau=knownParam(6);
elseif length(knownParam)>6
    MatProps.Gi=knownParam(5:(end-length(constParam)));
    MatProps.tau=knownParam((end-length(constParam)+1):end);
end
