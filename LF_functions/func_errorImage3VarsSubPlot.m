function [mapZ,minErr] = func_errorImage3VarsSubPlot(plotParams,xVar,yVar,zVar)
...
zInterp = scatteredInterpolant(xVar,yVar,zVar);
...
if abs(errMax) > abs(errMin)
    cBounds = [errMin,quantile(mapZ(:),0.85)];
else
    cBounds = [quantile(mapZ(:),0.15),errMax];
end
