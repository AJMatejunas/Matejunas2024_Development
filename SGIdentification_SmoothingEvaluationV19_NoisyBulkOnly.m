...
%V17_NoisyBulkOnly- 2023/03/21: Converted V17_NoisyShear
%V19_NoisyBulkOnly- 2023/03/28: Made spatial cropping and extrapolation
                                       %intervals the same
                   %2023/04/08: NO VERSION CHANGE removed exact shear
                                    %modulus inputs
...
%% Create structute of known parameters
    ...
    exactPropsE=exactProps;
    exactPropsI=exactProps;
    exactPropsE.Gi=RefPar.MatProps.Gi;
    exactPropsI.Gi=str2double(inputdlg('Previously Identified G'));
...
%% Run the identification for every copy smoothing Kernel
%initiaize cost function vector
[phiE,phiI]=deal(zeros(TempKernNum,SpaKernNum,NumNoise,3)); 
extrapOpts.accel.tempPadOn=false;

%% Initialize contitutive Parameter
ConstitutiveParamE=zeros(TempKernNum,SpaKernNum,NumNoise,2);
ConstitutiveParamI=zeros(TempKernNum,SpaKernNum,NumNoise,2);
Rho=material.rho;
timefull=time;
   ...
   if TempKernVec(k)==0
    ...
    for m=1:SpaKernNum
        ...
        parfor n=1:NumNoise
            ...
            [TempKI,TempTauI,TempPhiI,TempKE,TempTauE,TempPhiE]=...
                deal(zeros(3,1));
            for ig=1:3
                intGuess=squeeze(intMatrix(:,ig)); %#ok<PFBNS>

%                 %[TempParam,TempPhiE(ig)]=func_PronyBulkTauSGvfm(strain, ...
%                     time.vec,SG.x,exactPropsE,...
%                     intGuess,ub,lb,SolveOpts,minOpts,CondOpts);
%                 TempKE(ig)=TempParam(1);
%                 TempTauE(ig)=TempParam(2);

                [TempParam,TempPhiI(ig)]=func_PronyBulkTauSGvfm(strain, ...
                    time.vec,SG.x,exactPropsI,...
                    intGuess,ub,lb,SolveOpts,minOpts,CondOpts);
                TempKI(ig)=TempParam(1);
                TempTauI(ig)=TempParam(2);
            ...
            %phiE(k,m,n,:)=TempPhiE';
            %ConstitutiveParamE(k,m,n,:)=[KE(1);tauE(1)];

            minPhiI=min(TempPhiI);
            KI=TempKI(TempPhiI==minPhiI);
            tauI=TempTauI(TempPhiI==minPhiI);
            phiI(k,m,n,:)=TempPhiI';
            ConstitutiveParamI(k,m,n,:)=[KI(1);tauI(1)];
...
% Ident.K.E=squeeze(ConstitutiveParamE(:,:,:,1));
Ident.K.I=squeeze(ConstitutiveParamI(:,:,:,1));

% Ident.tau.E=squeeze(ConstitutiveParamE(:,:,:,2));
Ident.tau.I=squeeze(ConstitutiveParamI(:,:,:,2));

%% Calculate Identification Errors
Errors.K.I=(Ident.K.I...
    -RefPar.MatProps.Ki)/RefPar.MatProps.Ki*100;
% Errors.K.E=(Ident.K.E...
%     -RefPar.MatProps.Ki)/RefPar.MatProps.Ki*100;

% Errors.tau.E=(Ident.tau.E...
%     -RefPar.MatProps.tau)/RefPar.MatProps.tau*100;
Errors.tau.I=(Ident.tau.I...
    -RefPar.MatProps.tau)/RefPar.MatProps.tau*100;

%% Calcualte error statistics
%Bulk Modulus
sysErr.K.I=squeeze(mean(Errors.K.I,3));
ranErr.K.I=squeeze(std(Errors.K.I,0,3));
totErr.K.I=abs(sysErr.K.I)+2*ranErr.K.I;
% 
% sysErr.K.E=squeeze(mean(Errors.K.E,3));
% ranErr.K.E=squeeze(std(Errors.K.E,0,3));
% totErr.K.E=abs(sysErr.K.E)+2*ranErr.K.E;

% Time Constant
sysErr.tau.I=squeeze(mean(Errors.tau.I,3));
ranErr.tau.I=squeeze(std(Errors.tau.I,0,3));
totErr.tau.I=abs(sysErr.tau.I)+2*ranErr.tau.I;

% sysErr.tau.E=squeeze(mean(Errors.tau.E,3));
% ranErr.tau.E=squeeze(std(Errors.tau.E,0,3));
% totErr.tau.E=abs(sysErr.tau.E)+2*ranErr.tau.E;

%% Save Results
    save(strcat(savePath,'/',TestDeg,...
        '_CombinedSpatialTemporal_Noisy','_BulkTauOnly.mat'),...
        'intMatrix','ub','lb','ConstitutiveParamE','ConstitutiveParamI', ...
        'SolveOpts','imageNoise','exactPropsI','exactPropsE',...
        'CondOpts','Errors','smoothOpts','TempKernVec','SpaKernVec', ...
        'phiI','phiE','extrapOpts','Ident','sysErr','ranErr','totErr');

%% Plot Systematic Results image SC
figure('units','Centimeters','outerposition',[0 0 17.8 19])
% subplot(2,2,1)
% imagesc(SpaKernVec,TempKernVec,sysErr.K.E)
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('K_1 Error_{sys} Exact G (%)')
% colormap(PBWYRK_map)
% colorbar
% set(gca,'YDir','normal')
% caxis([-25,25])
% 
% subplot(2,2,2)
% imagesc(SpaKernVec,TempKernVec,sysErr.tau.E)
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('\tau_1 Error_{sys} Exact G (%)')
% colormap(PBWYRK_map)
% colorbar
% set(gca,'YDir','normal')
% caxis([-25,25])
...
subplot(1,2,1)
imagesc(SpaKernVec,TempKernVec,sysErr.K.I)
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('K_1 Error_{sys} Identified G (%)')
colormap(PBWYRK_map)
colorbar
set(gca,'YDir','normal')
caxis([-25,25])
...
subplot(1,2,2)
imagesc(SpaKernVec,TempKernVec,sysErr.tau.I)
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
...
caxis([-5,5])
FigSaveName=strcat(savePath,'/',TestDeg,...
    '_NoNoise_Konly_sysErr_imagesc_zoomed');
...
% subplot(2,2,1)
% contourf(SpaKernVec,TempKernVec,sysErr.K.E,50,'LineStyle','none')
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('K_1 Err_{sys} Exact G (%)')
% colormap(PBWYRK_map)
% clim([-25,25])
% colorbar
% 
% subplot(2,2,2)
% contourf(SpaKernVec,TempKernVec,sysErr.tau.E,50,'LineStyle','none')
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('\tau_1 Err_{sys} Exact G (%)')
% colormap(PBWYRK_map)
% clim([-25,25])
% colorbar

subplot(1,2,1)
contourf(SpaKernVec,TempKernVec,sysErr.K.I,50,'LineStyle','none')
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('K_1 Err_{sys} Identified G (%)')
colormap(PBWYRK_map)
clim([-25,25])
colorbar

subplot(1,2,2)
contourf(SpaKernVec,TempKernVec,sysErr.tau.I,50,'LineStyle','none')
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
...
FigSaveName=strcat(savePath,'/',TestDeg,'_NoNoise_Konly_sysErr_PBWYRK');
saveas(gcf,FigSaveName,'fig')
saveas(gcf,FigSaveName,'svg')
...
figure('units','Centimeters','outerposition',[0 0 17.8 19])
% subplot(2,2,1)
% imagesc(SpaKernVec,TempKernVec,ranErr.K.E)
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('K_1 Error_{ran} Exact G (%)')
% colormap(PBWYRK_map)
% colorbar
% set(gca,'YDir','normal')
% caxis([-25,25])
% 
% subplot(2,2,2)
% imagesc(SpaKernVec,TempKernVec,ranErr.tau.E)
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('\tau_1 Error_{ran} Exact G (%)')
% colormap(PBWYRK_map)
% colorbar
% set(gca,'YDir','normal')
% caxis([-25,25])
...
subplot(1,2,1)
imagesc(SpaKernVec,TempKernVec,ranErr.K.I)
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('K_1 Error_{ran} Identified G (%)')
colormap(PBWYRK_map)
colorbar
set(gca,'YDir','normal')
caxis([-25,25])
...
subplot(1,2,2)
imagesc(SpaKernVec,TempKernVec,ranErr.tau.I)
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
...
caxis([-5,5])
FigSaveName=strcat(savePath,'/',TestDeg,...
    '_NoNoise_Konly_ranErr_imagesc_zoomed');
...
% subplot(2,2,1)
% contourf(SpaKernVec,TempKernVec,ranErr.K.E,50,'LineStyle','none')
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('K_1 Err_{ran} Exact G (%)')
% colormap(PBWYRK_map)
% clim([-25,25])
% colorbar
% 
% subplot(2,2,2)
% contourf(SpaKernVec,TempKernVec,ranErr.tau.E,50,'LineStyle','none')
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('\tau_1 Err_{ran} Exact G (%)')
% colormap(PBWYRK_map)
% clim([-25,25])
% colorbar

subplot(1,2,1)
contourf(SpaKernVec,TempKernVec,ranErr.K.I,50,'LineStyle','none')
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('K_1 Err_{ran} Identified G (%)')
colormap(PBWYRK_map)
clim([-25,25])
colorbar

subplot(1,2,2)
contourf(SpaKernVec,TempKernVec,ranErr.tau.I,50,'LineStyle','none')
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
...
FigSaveName=strcat(savePath,'/',TestDeg,'_NoNoise_Konly_ranErr_PBWYRK');
saveas(gcf,FigSaveName,'fig')
saveas(gcf,FigSaveName,'svg')
%% Plot total Results image SC
figure('units','Centimeters','outerposition',[0 0 17.8 19])
% subplot(2,2,1)
% imagesc(SpaKernVec,TempKernVec,totErr.K.E)
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('K_1 Error_{tot} Exact G (%)')
% colormap(PBWYRK_map)
% colorbar
% set(gca,'YDir','normal')
% caxis([-25,25])
% 
% subplot(2,2,2)
% imagesc(SpaKernVec,TempKernVec,totErr.tau.E)
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('\tau_1 Error_{tot} Exact G (%)')
% colormap(PBWYRK_map)
% colorbar
% set(gca,'YDir','normal')
% clim([-25,25])
...
subplot(1,2,1)
imagesc(SpaKernVec,TempKernVec,totErr.K.I)
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('K_1 Error_{tot} Identified G (%)')
colormap(PBWYRK_map)
colorbar
set(gca,'YDir','normal')
caxis([-25,25])
...
subplot(1,2,2)
imagesc(SpaKernVec,TempKernVec,totErr.tau.I)
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
...
caxis([-5,5])
FigSaveName=strcat(savePath,'/',TestDeg,...
    '_NoNoise_Konly_totErr_imagesc_zoomed');
...
% subplot(2,2,1)
% contourf(SpaKernVec,TempKernVec,totErr.K.E,50,'LineStyle','none')
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('K_1 Err_{tot} Exact G (%)')
% colormap(PBWYRK_map)
% clim([-25,25])
% colorbar
% 
% subplot(2,2,2)
% contourf(SpaKernVec,TempKernVec,totErr.tau.E,50,'LineStyle','none')
% xlabel('Spatial Kernel (pixels)')
% ylabel('Temporal Kernel (frames)')
% title('\tau_1 Err_{tot} Exact G (%)')
% colormap(PBWYRK_map)
% clim([-25,25])
% colorbar

subplot(1,2,1)
contourf(SpaKernVec,TempKernVec,totErr.K.I,50,'LineStyle','none')
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
title('K_1 Err_{tot} Identified G (%)')
colormap(PBWYRK_map)
clim([-25,25])
colorbar

subplot(1,2,2)
contourf(SpaKernVec,TempKernVec,totErr.tau.I,50,'LineStyle','none')
xlabel('Spatial Kernel (pixels)')
ylabel('Temporal Kernel (frames)')
...
FigSaveName=strcat(savePath,'/',TestDeg,'_NoNoise_Konly_totErr_PBWYRK');
saveas(gcf,FigSaveName,'fig')
saveas(gcf,FigSaveName,'svg')
