% This script was created to evaluate the strain and displacement 
    %corrections at the edges with moderate smoothing 11px spatial and 11 
    %frames temporal 
...
if exist([initPath,initFile],'file') ~= 2
    hWarn = warndlg('Processing parameter file does not exist.', ...
        'Processing parameters not found');
    waitfor(hWarn);
...
FEValidMode=false;
            ...
            switch    DispCorr.Opt
                case 'Yes'
                    ...
                    [disp,DispCorr,grid,ProgramVersions]=func_CorrectGMDispV3(disp,...
                        DispCorr,grid,pos);
            ...
            % displacements from images or displacements from FE data
                ...
                fprintf('Correcting Kinematic fields \n')
                 [strain,accel]=func_PostCorrectFields(grid,strainRaw, ...
                     RawAccel,pos,time,DispCorr,...
                    smoothingOpts);
